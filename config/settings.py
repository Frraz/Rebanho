...
SECRET_KEY = config(
    "SECRET_KEY", default="django-insecure-CHANGE-THIS-IN-PRODUCTION-abc123xyz789"
)
...
# Formatos de data — apenas data, sem hora (USE_L10N=True aplica estes
# no admin e em contextos sem localização ativa)
DATE_FORMAT = "d/m/Y"
SHORT_DATE_FORMAT = "d/m/Y"

# ✅ ALTERADO: removido H:i — sistema exibe apenas datas nos formulários
# Campos de auditoria (created_at, cancelled_at) mantêm DateTimeField
# internamente, mas a exibição ao usuário é padronizada sem hora
...
STOCK_BALANCE_RECONCILIATION_ENABLED = config(
    "STOCK_BALANCE_RECONCILIATION_ENABLED", default=True, cast=bool
)
STOCK_BALANCE_AUTO_RECONCILE_ON_INCONSISTENCY = config(
    "STOCK_BALANCE_AUTO_RECONCILE_ON_INCONSISTENCY", default=False, cast=bool
)
...
REPORT_PDF_GENERATION_TIMEOUT = config(
    "REPORT_PDF_GENERATION_TIMEOUT", default=60, cast=int
)
...
NOTIFICATION_EMAIL_PREFIX = config(
    "NOTIFICATION_EMAIL_PREFIX", default=f"[{SITE_NAME}]"
)
...
if not DEBUG:
    ...
    for cfg in critical_configs:
        if (
            cfg == "SECRET_KEY"
            and SECRET_KEY == "django-insecure-CHANGE-THIS-IN-PRODUCTION-abc123xyz789"
        ):
